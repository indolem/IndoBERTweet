...
test_f1s = 0.0
for idx in range(5):
    trainset = pd.read_csv(args.data_path+'train'+str(idx)+'.csv', encoding = "ISO-8859-1")
    devset = pd.read_csv(args.data_path+'dev'+str(idx)+'.csv', encoding = "ISO-8859-1")
    testset = pd.read_csv(args.data_path+'test'+str(idx)+'.csv', encoding = "ISO-8859-1")
    
    xtrain, ytrain = list(trainset['Tweet']), list(trainset['Label'])
    xdev, ydev = list(devset['Tweet']), list(devset['Label'])
    xtest, ytest = list(testset['Tweet']), list(testset['Label'])
